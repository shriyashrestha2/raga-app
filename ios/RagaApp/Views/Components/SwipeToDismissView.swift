...
/// Wraps a row with a left-swipe-to-clear gesture. `.swipeActions` only works
/// inside a `List`, but this feed is a plain `ForEach` in a `ScrollView`, so
/// this reimplements the gesture by hand. This is a personal, client-side
/// dismiss (not a backend delete) — it just hides the item from this
/// device's feed going forward, so anyone can declutter notifications
/// they're done with regardless of delete permission.
///
                ...
                .simultaneousGesture(
                    DragGesture(minimumDistance: 15)
                        .onChanged { value in
                            ...
                            offset = min(0, max(translation.width, -rowWidth))
                        }
                        .onEnded { value in
                            let translation = value.translation
